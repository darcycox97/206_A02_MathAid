...
public class MathsAid extends JFrame implements CreationWorkerListener {
	...
	private CreationWorker _worker;
	...
	public MathsAid() {
		...
		_btnCreateMode.setFont(_btnCreateMode.getFont().deriveFont((float)15));
		_btnDelete = new JButton(DELETE_BUTTON);
		_btnDelete.setFont(_btnDelete.getFont().deriveFont((float)15));
		_btnPlay = new JButton(PLAY_BUTTON);
		_btnPlay.setFont(_btnPlay.getFont().deriveFont((float)15));
		...
		_lblExisting.setBorder(BorderFactory.createEmptyBorder(5, 0, 5, 0));

		initializeCreationsList(); // sorts the creations by time modified, and adds them in that order
		_listExisting = new JList<Creation>(_existingCrtns);
		_listExisting.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		_listExisting.setFont(_listExisting.getFont().deriveFont((float)16));
		...
		_btnCreate.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				if (_btnCreate.getText().equals(CREATE_BUTTON)) {
					...
					// in this case the audio component is generated by the CreationWorker
					// the remaining components are created in the audioComponentCreated() method once the worker
					// completes executing.
					_worker = new CreationWorker(_crtnToGenerate, ERROR_MSG);
					_worker.addCreationWorkerListener(MathsAid.this);
					_worker.execute();
				}
			}
		});
	...
	public void recording() {
		...
		_lblCreateModeStatus.setText(STATUS_RECORDING);
	}
